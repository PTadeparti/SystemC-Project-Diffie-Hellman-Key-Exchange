...
SC_MODULE(datapath)
{
	...
	SC_CTOR(datapath) : R0in("R0in"), R1in("R1in"), R2in("R2in"), R3in("R3in"), R0out("R0out"), R1out("R1out"), R2out("R2out"), RP("RP"), BP("BP")
	{
	    ...
	    RP.t0(t0inRP); RP.t1(t1inRP); RP.c(cin); RP.aHigh(aHighin); RP.clock(clock); RP.out1(t0outRP); RP.out2(t1outRP);
	    BP.T0(t0outRP); BP.T1(t1outRP); BP.C(cin); BP.AH(aHighin); BP.clock(clock); BP.ready(readyBP); BP.T0new(t0outBP); BP.T1new(t1outBP); BP.AHnew(aHighOutBP);
	    
	    R0out.a(t0outBP); R0out.load(loadOutput); R0out.clock(clock); R0out.out(to_sw0);
	    R1out.a(t1outBP); R1out.load(loadOutput); R1out.clock(clock); R1out.out(to_sw1);
	    R2out.a(aHighOutBP); R2out.load(loadOutput); R2out.clock(clock); R2out.out(to_sw2);
